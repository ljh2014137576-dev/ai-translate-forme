...
一个基于 Manifest V3 的 Chrome 扩展：把整个网页的可见文本抽取出来，调用主流 AI 大模型（DeepSeek / OpenAI / Claude / Gemini / 通义千问 / Kimi / 智谱 / 豆包）翻译为目标语言，再回填到页面上，并支持一键还原原文。

版本：v0.3.0（纯原生 JS，无外部依赖）

## 功能简介

- 一键翻译整页可见文本（自动跳过代码、输入框、隐藏元素等），三种显示模式：原文 / 译文 / 双语并存
- 相同文本去重后只请求一次，节省 token
- 文本按字符数分批、并发请求，失败自动重试 1 次
- 一键「还原原文」；页面翻译缓存（检测页面变更，未变更直接套用缓存），可导出/导入为本地 JSON 文件
- 设置页（Google Material 风格）可选 8 家服务商，自动带出官方定价做费用估算；API 返回 cost 字段时优先采用实际费用
- token 用量统计（设置页查看/重置）；快捷键 `Alt+T` 直接翻译当前页
